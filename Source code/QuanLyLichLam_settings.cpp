...
void sua_CongViec(Nodeptr& list, char* ten) {
	...
	while (p != NULL) {
		if (strcmp(p->data.ten, ten) == 0) {
			...
			//Chuan hoá chuoi
			char str1[50];
			for (int i = 0; i < strlen(p->data.ten); i++) {
				str1[i] = tolower(p->data.ten[i]);
			}
			str1[strlen(p->data.ten)] = '\0';
			xoaktRong(str1); 
			for (int i = 0; i < strlen(p->data.ten); i++) {
				p->data.ten[i] = str1[i];
			}
...
void capNhatTrangThai(Nodeptr& list, char* ten) {
	Nodeptr p = list;
	bool isFound = false;
	while (p != NULL) {
		if (strcmp(p->data.ten, ten) == 0) {
			isFound = true;
			cout << "Cap nhat trang thai cua cong viec " << ten << " thanh:";
	...
	if (!isFound) {
		cout << "Khong tim thay cong viec co ten " << ten << " de cap nhat trang thai.\n";
	}
}
// 3. Xoá
